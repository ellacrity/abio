...
use crate::contiguous::Span;
use crate::layout::endian::{BigEndian, Endian, LittleEndian};
use crate::Decode;
    ...
    ($($ty:ty: $decoder:ident),* $(,)?) => {
        $(
            impl Decode for $ty {
                ...
                fn decode<E: $crate::endian::Endian>(bytes: &[u8], offset: Self::Offset, endian: E) -> $crate::Result<Self> {
                    if E::is_big_endian(&endian) {
                        LittleEndian::$decoder(bytes, offset.start())
                    } else {
                        BigEndian::$decoder(bytes, offset.start())
                    }
                }
            }
        )*
